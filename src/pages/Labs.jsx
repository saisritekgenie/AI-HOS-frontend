        ...
        <div className="modal-overlay">
          <div className="modal-card" style={{ maxWidth: "520px", padding: "2rem", borderRadius: "20px", background: "var(--bg-secondary)", border: "1px solid var(--border-glass)", boxShadow: "0 20px 40px rgba(0,0,0,0.4)" }}>
            <div style={{ textAlign: "center", borderBottom: "1px dashed rgba(255,255,255,0.15)", paddingBottom: "1rem", marginBottom: "1.25rem" }}>
              <h4 style={{ margin: 0, fontSize: "1.25rem", fontWeight: 800, color: "var(--text-primary)", letterSpacing: "0.5px" }}>LABORATORY REPORT DETAILS</h4>
              <span style={{ fontSize: "0.8rem", color: "var(--text-secondary)", fontWeight: 600 }}>Test Request ID: {viewingLab._id.slice(-6).toUpperCase()}</span>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem", fontSize: "0.9rem", marginBottom: "1.25rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <strong style={{ color: "var(--text-primary)", fontWeight: 700 }}>{viewingLab.testName}</strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "var(--text-secondary)", fontWeight: 500 }}>Patient Name:</span>
                <strong style={{ color: "var(--text-primary)", fontWeight: 700 }}>{viewingLab.patient?.firstName} {viewingLab.patient?.lastName}</strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "var(--text-secondary)", fontWeight: 500 }}>UHID (Patient ID):</span>
                <strong style={{ color: "var(--text-primary)", fontWeight: 700 }}>{viewingLab.patient?.uhid || "N/A"}</strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "var(--text-secondary)", fontWeight: 500 }}>Ordered By:</span>
                <strong style={{ color: "var(--text-primary)", fontWeight: 700 }}>Dr. {viewingLab.prescribedBy?.firstName} {viewingLab.prescribedBy?.lastName}</strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "var(--text-secondary)", fontWeight: 500 }}>Sample Collected At:</span>
                <span style={{ color: "var(--text-primary)", fontWeight: 600 }}>{viewingLab.sampleCollectedAt ? new Date(viewingLab.sampleCollectedAt).toLocaleString() : "Pending Collection"}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px dashed rgba(255,255,255,0.15)", paddingBottom: "0.75rem" }}>
                ...
                <strong style={{ 
                  ...
                  fontWeight: 800, 
                  letterSpacing: "0.5px",
                  background: viewingLab.status === "COMPLETED" ? "rgba(34, 197, 94, 0.15)" : viewingLab.status === "REJECTED" ? "rgba(239, 68, 68, 0.15)" : "rgba(234, 179, 8, 0.15)",
                  color: viewingLab.status === "COMPLETED" ? "#4ade80" : viewingLab.status === "REJECTED" ? "#f87171" : "#facc15",
                  border: viewingLab.status === "COMPLETED" ? "1px solid rgba(74, 222, 128, 0.3)" : viewingLab.status === "REJECTED" ? "1px solid rgba(248, 113, 113, 0.3)" : "1px solid rgba(250, 204, 21, 0.3)"
                }}>
            ...
            <div style={{ background: "rgba(255, 255, 255, 0.04)", padding: "1rem", borderRadius: "12px", border: "1px solid var(--border-glass)", marginBottom: "1.25rem" }}>
              <h5 style={{ margin: "0 0 0.5rem 0", fontSize: "0.85rem", color: "var(--accent-primary)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px" }}>DIAGNOSTIC FINDINGS / RESULTS</h5>
