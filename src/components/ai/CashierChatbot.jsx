...
import { fetchCashierAIChat } from "../../services/api";
import { AIVoiceAssistant } from "../common/AIVoiceAssistant";

export const CashierChatbot = ({ activeTab }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    { 
      sender: "bot", 
      text: "Hello! I am your AI Billing Assistant. I can help you analyze revenue logs, check outstanding balances, compose payment reminders, or review transaction channels. How can I help you today?",
      takeaways: [],
      recommendations: ["I assist with billing analytics and cash collection coordination."]
    }
  ]);
  ...
  const handleSend = async (textToSend = inputVal) => {
    ...
    try {
      const res = await fetchCashierAIChat(textToSend, activeTab);
      
      const botMsg = {
        sender: "bot",
        text: res.data.reply || "Billing review completed.",
      ...
      setMessages(prev => [...prev, { 
        sender: "bot", 
        text: "Sorry, I encountered an issue connecting to the cashier AI companion. Verify session roles.",
        takeaways: [],
        recommendations: []
      }]);
    } finally {
      ...
      {isOpen && (
        <div
          style={{
            ...
            background: "white",
            borderRadius: "20px",
            boxShadow: "0 12px 40px rgba(190, 24, 93, 0.22)",
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              ...
              <div>
                <h4 style={{ margin: 0, fontSize: "0.95rem", fontWeight: 800 }}>Billing AI Desk</h4>
                <span style={{ fontSize: "0.65rem", color: "#fbcfe8", display: "flex", alignItems: "center", gap: "0.2rem" }}>
                  <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#10b981", display: "inline-block" }}></span>
                  Settlement & Payments Analyzer
                </span>
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              style={{ background: "none", border: "none", color: "#fbcfe8", cursor: "pointer" }}
            >
              <X size={18} />
            </button>
          </div>
          ...
          <div style={{ flex: 1, padding: "1.25rem", overflowY: "auto", display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              >
                <div
                  style={{
                    ...
                    background: m.sender === "user" ? "#db2777" : "#f1f5f9",
            ...
            {loading && (
              <div style={{ display: "flex", alignSelf: "flex-start", gap: "0.5rem", alignItems: "center", background: "#f8fafc", padding: "0.5rem 0.75rem", borderRadius: "12px" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748b", fontWeight: 600 }}>Calculating revenue logs...</span>
              </div>
            )}
          ...
          <div style={{ padding: "0.25rem 1rem", display: "flex", gap: "0.4rem", flexWrap: "wrap", borderTop: "1px solid #f1f5f9" }}>
            <button 
              onClick={() => handleSend("Analyze current cashier settled revenue metrics.")} 
              style={{ border: "1px solid #e2e8f0", background: "none", borderRadius: "12px", padding: "0.25rem 0.5rem", fontSize: "0.65rem", cursor: "pointer", color: "#be185d" }}
            >
              💰 Revenue summary
            </button>
            <button 
              onClick={() => handleSend("Draft outstanding payment SMS reminders.")} 
              style={{ border: "1px solid #e2e8f0", background: "none", borderRadius: "12px", padding: "0.25rem 0.5rem", fontSize: "0.65rem", cursor: "pointer", color: "#be185d" }}
            >
              💬 Draft collection SMS
            </button>
          </div>
