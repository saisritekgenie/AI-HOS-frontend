...
import { fetchLabTechnicianAIChat } from "../../services/api";
import { AIVoiceAssistant } from "../common/AIVoiceAssistant";

export const LabTechnicianChatbot = ({ activeTab }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    { 
      sender: "bot", 
      text: "Hello! I am your AI Lab Assistant. I can help you summarize diagnostic report contents, flag abnormal blood index values, or check standard test ranges. How can I help you today?",
      takeaways: [],
      recommendations: ["I assist with clinical laboratory findings validation."]
    }
  ]);
  ...
  const handleSend = async (textToSend = inputVal) => {
    ...
    try {
      const res = await fetchLabTechnicianAIChat(textToSend, activeTab);
      
      const botMsg = {
        sender: "bot",
        text: res.data.reply || "Lab report analysis complete.",
      ...
      setMessages(prev => [...prev, { 
        sender: "bot", 
        text: "Sorry, I encountered an issue connecting to the laboratory AI companion. Check server permissions.",
        takeaways: [],
        recommendations: []
      }]);
    } finally {
      ...
      {isOpen && (
        <div
          style={{
            ...
            background: "white",
            borderRadius: "20px",
            boxShadow: "0 12px 40px rgba(15, 118, 110, 0.22)",
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              ...
              <div>
                <h4 style={{ margin: 0, fontSize: "0.95rem", fontWeight: 800 }}>Laboratory AI Desk</h4>
                <span style={{ fontSize: "0.65rem", color: "#ccfbf1", display: "flex", alignItems: "center", gap: "0.2rem" }}>
            ...
            <button
              ...
              style={{ background: "none", border: "none", color: "#ccfbf1", cursor: "pointer" }}
            >
              <X size={18} />
            </button>
          </div>
          ...
          <div style={{ flex: 1, padding: "1.25rem", overflowY: "auto", display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              >
                <div
                  style={{
                    ...
                    background: m.sender === "user" ? "#0d9488" : "#f1f5f9",
            ...
            {loading && (
              <div style={{ display: "flex", alignSelf: "flex-start", gap: "0.5rem", alignItems: "center", background: "#f8fafc", padding: "0.5rem 0.75rem", borderRadius: "12px" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748b", fontWeight: 600 }}>Analyzing lab diagnostics...</span>
              </div>
            )}
          ...
          <div style={{ padding: "0.25rem 1rem", display: "flex", gap: "0.4rem", flexWrap: "wrap", borderTop: "1px solid #f1f5f9" }}>
            <button 
              onClick={() => handleSend("Explain hemoglobin (Hb) reference levels.")} 
              style={{ border: "1px solid #e2e8f0", background: "none", borderRadius: "12px", padding: "0.25rem 0.5rem", fontSize: "0.65rem", cursor: "pointer", color: "#0f766e" }}
            >
              🔬 Hb reference ranges
            </button>
            <button 
              onClick={() => handleSend("Flag blood sugar glucose ranges criteria.")} 
              style={{ border: "1px solid #e2e8f0", background: "none", borderRadius: "12px", padding: "0.25rem 0.5rem", fontSize: "0.65rem", cursor: "pointer", color: "#0f766e" }}
            >
              🩸 Glucose check criteria
            </button>
          </div>
          ...
          <div style={{ padding: "0.75rem 1rem", borderTop: "1px solid #f1f5f9", display: "flex", gap: "0.5rem", alignItems: "center" }}>
            ...
            <input
              ...
              placeholder="Query laboratory AI..."
              style={{
                flex: 1,
