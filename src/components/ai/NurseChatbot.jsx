...
import { fetchNurseAIChat } from "../../services/api";
import { AIVoiceAssistant } from "../common/AIVoiceAssistant";

export const NurseChatbot = ({ activeTab }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    { 
      sender: "bot", 
      text: "Hello! I am your AI Nursing Assistant. I can help you evaluate critical vitals logs, check medication administration reminders, and trace emergency patient alerts. How can I help you today?",
      takeaways: [],
      recommendations: ["I assist in monitoring patient vitals safety margins."]
    }
  ]);
  ...
  const handleSend = async (textToSend = inputVal) => {
    ...
    try {
      const res = await fetchNurseAIChat(textToSend, activeTab);
      
      const botMsg = {
        sender: "bot",
        text: res.data.reply || "Nursing logs check completed.",
      ...
      setMessages(prev => [...prev, { 
        sender: "bot", 
        text: "Sorry, I encountered an issue connecting to the nursing AI assistant. Please check credentials.",
        takeaways: [],
        recommendations: []
      }]);
    } finally {
      ...
      {isOpen && (
        <div
          style={{
            ...
            background: "white",
            borderRadius: "20px",
            boxShadow: "0 12px 40px rgba(67, 56, 202, 0.22)",
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              ...
              <div>
                <h4 style={{ margin: 0, fontSize: "0.95rem", fontWeight: 800 }}>Nursing AI Desk</h4>
                <span style={{ fontSize: "0.65rem", color: "#c7d2fe", display: "flex", alignItems: "center", gap: "0.2rem" }}>
          ...
          <div style={{ flex: 1, padding: "1.25rem", overflowY: "auto", display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            {loading && (
              <div style={{ display: "flex", alignSelf: "flex-start", gap: "0.5rem", alignItems: "center", background: "#f8fafc", padding: "0.5rem 0.75rem", borderRadius: "12px" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748b", fontWeight: 600 }}>Analyzing vitals safety limits...</span>
              </div>
            )}
          ...
          <div style={{ padding: "0.25rem 1rem", display: "flex", gap: "0.4rem", flexWrap: "wrap", borderTop: "1px solid #f1f5f9" }}>
            <button 
              onClick={() => handleSend("What are the critical vitals safety triggers?")} 
              style={{ border: "1px solid #e2e8f0", background: "none", borderRadius: "12px", padding: "0.25rem 0.5rem", fontSize: "0.65rem", cursor: "pointer", color: "#4f46e5" }}
            >
              🚨 Critical Alert Safety Check
            </button>
            <button 
              onClick={() => handleSend("Describe pending medication due logs routines.")} 
              style={{ border: "1px solid #e2e8f0", background: "none", borderRadius: "12px", padding: "0.25rem 0.5rem", fontSize: "0.65rem", cursor: "pointer", color: "#4f46e5" }}
            >
              💊 Due Meds Routine
            </button>
